...
class ChatModel:
    def __init__(self, model_id: str = "google/gemma-2b-it", device="cpu"):
        ACCESS_TOKEN = os.getenv("ACCESS_TOKEN")  # Ensure this is set in your .env file

        self.tokenizer = AutoTokenizer.from_pretrained(
            model_id, cache_dir=CACHE_DIR, token=ACCESS_TOKEN
        )

        # Set up quantization config only if a GPU is available
        ...
        self.model.to(device)  # Ensure the model is loaded onto the correct device
        self.model.eval()
        self.chat = []
        self.device = device
